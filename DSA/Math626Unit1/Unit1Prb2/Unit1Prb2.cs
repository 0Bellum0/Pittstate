...
namespace Unit1Smp1
{
    class Unit1Smp1
    {
        ...
        static int[] Sort()
        {
            int i, j, min;
            int aux;
            int[] printOrder = new int[numOfStudents];
            ...
            for (i = 0; i < numOfStudents; i++)
            {
                ...
                {
                    if (string.Compare((lastNameArray[printOrder[min]] + firstNameArray[printOrder[min]]), (lastNameArray[printOrder[j]] + firstNameArray[printOrder[j]])) > 0)
                    {
                        min = j;
                    }
                }
        ...
        static void PrintResults(int[] printOrder)
        {
            int i;
